...
export default function Cart() {
  ...
  async function getCart() {
    ...
    if (res.status === 200) return setItems(data);
    if (res.status === 500) return console.log('internal server error')
  }
  ...
  function hapusItems() {
    if (checked.length > 0) {
      fetch('http://localhost:4000/api/v1/cart', {
        ...
        if (res.status === 200) {
          setItems(prev => prev.filter((item) => item.id !== checked.find(id => id === item.id )))
          setChecked([])
        }
      })
    } else { setToast({
      open: true,
      msg: 'pilih item terlebih dahulu',
      bgColor: 'error.main'
    })}
  }
  ...
  function checkout() {
    if (!checked.length) {
      return setToast({
        open: true,
        msg: 'pilih item yg mau dicheckout!!',
        bgColor: 'error.main'
      })
    }
    const state = items.filter(item => item.id === checked.find(id => id === item.id))
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button onClick={hapusItems} variant='outlined' color='error' sx={{mr:{sm:'auto'}}}>Hapus</Button>
        <Box sx={{ display: 'flex', alignItems: 'center', gap:'8px', mt:{xs:'10px', sm:0}, pl:1 }}>
